...
Page({
  ...
  against: function (event) {
    ...
    if (that.data.against.status === 0) {
      ...
      animation.scale(2).step()
      animation.scale(1).step()
      that.data.against.animationData = animation.export()
    }

    that.data.against.status === 0 ? p = 1 : p = 0
    that.data.against.status = p
    that.setData({
      against: that.data.against
    })
    that.data.against.animationData = {}
    that.setData({
      against: that.data.against
    })
  },
